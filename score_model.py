...
class GlobalEncoder(torch.nn.Module):
    def __init__(self, emb_dim, global_dim):
        super(GlobalEncoder, self).__init__()
        self.emb = torch.nn.Embedding(global_dim, emb_dim)
        self.global_dim = global_dim
    
    def forward(self, x):
        # We should just have a scalar that gets passed in
        x_emb = 0
        for i in range(self.global_dim):
            x_emb += self.emb(x[:,i].long())
        return x_emb
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
        ...
        self.gl_edge_dim = 1
        self.g_embed_dim = 1
        self.g_node_embedding = GlobalEncoder(emb_dim = ns, global_dim = self.g_embed_dim)
        ...
        self.gl_edge_embedding = nn.Sequential(nn.Linear(self.gl_edge_dim, ns), nn.ReLU(), nn.Dropout(dropout),nn.Linear(ns, ns))
        ...
        lig_conv_layers, rec_conv_layers, lig_to_rec_conv_layers, rec_to_lig_conv_layers, lig_to_g_conv_layers, g_to_lig_conv_layers = [], [], [], [], [], []
        for i in range(num_conv_layers):
            ...
            lig_to_g_layer = TensorProductConvLayer(**parameters)
            lig_to_g_conv_layers.append(lig_to_g_layer)
            g_to_lig_layer = TensorProductConvLayer(**parameters)
            g_to_lig_conv_layers.append(g_to_lig_layer)
        ...
        self.lig_to_g_conv_layers = nn.ModuleList(lig_to_rec_conv_layers)
        self.g_to_lig_conv_layers = nn.ModuleList(g_to_lig_conv_layers)
    ...
    def forward(self, data):
        batch_size = data['ligand'].x.shape[0]
        ...
        # build global graph
        g_node_attr = self.build_g_conv_graph(batch_size)
        g_node_attr = self.g_node_embedding(g_node_attr)
        ...
        gl_index, gl_attr, gl_sh = self.build_cross_g_graph(data, batch_size, self.gl_edge_dim)
        cross_g, cross_g_lig = gl_index
        
        gl_attr = self.gl_edge_embedding(gl_attr)
        ...
        for l in range(len(self.lig_conv_layers)):
            ...
            # global to ligand message passing
            
            cross_g_lig = torch.tensor([min(lig_node_attr.shape[0], edge) for edge in list(cross_g_lig)], dtype = torch.int64)
            g_to_lig_edge_attr_ = torch.cat([gl_attr, lig_node_attr[cross_g_lig, :self.ns], g_node_attr[cross_g, :self.ns]], -1)
            lig_g_update = self.g_to_lig_conv_layers[l](g_node_attr, gl_index, g_to_lig_edge_attr_, gl_sh,
                                                              out_nodes=lig_node_attr.shape[0])
            ...
            if l != len(self.lig_conv_layers) - 1:
                ...
                lig_to_g_edge_attr_ = torch.cat([gl_attr, lig_node_attr[cross_g_lig, :self.ns], g_node_attr[cross_g, :self.ns]], -1)
                g_inter_update = self.lig_to_g_conv_layers[l](lig_node_attr, torch.flip(gl_index, dims=[0]), lig_to_g_edge_attr_,
                                                                  gl_sh, out_nodes=g_node_attr.shape[0])
            ...
            lig_node_attr = lig_node_attr + lig_intra_update + lig_inter_update + lig_g_update
            ...
            if l != len(self.lig_conv_layers) - 1:
                ...
                g_node_attr = F.pad(g_node_attr, (0, g_inter_update.shape[-1] - g_node_attr.shape[-1]))
                g_node_attr = g_node_attr + g_inter_update
    ...
    def build_g_conv_graph(self, batch_size):
    ...
    def build_cross_g_graph(self, data, batch_size, edge_dim = 1):
        num_atoms = len(data['ligand'])
        src = torch.zeros((num_atoms), dtype = torch.int64)
        dst = torch.arange((num_atoms), dtype = torch.int64)
        edge_attr = torch.randn(num_atoms)
        edge_sh = o3.spherical_harmonics(self.sh_irreps, torch.ones(num_atoms, 3), normalize=True, normalization='component')
        return torch.stack((src, dst)), edge_attr.reshape(num_atoms, edge_dim), edge_sh
